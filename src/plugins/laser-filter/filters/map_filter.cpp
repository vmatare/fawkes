...
LaserMapFilterDataFilter::load_map()
{
	...
	fawkes::amcl::read_map_config(config_,
	                              cfg_map_file,
	                              cfg_resolution,
	                              cfg_origin_x,
	                              cfg_origin_y,
...
{
	if (cell_x < 0 || cell_x > map_->size_x || cell_y < 0 || cell_y > map_->size_y) {
		return false;
	}
	return true;
}
...
LaserMapFilterDataFilter::filter()
{
	...
	for (unsigned int a = 0; a < vecsize; ++a) {
		...
		try {
			tf_listener_->lookup_transform(frame_map_.c_str(),
			                               in[a]->frame,
			                               *(in[a]->timestamp),
			                               transform);
			...
			try {
				...
				logger_->log_warn("map_filter",
				                  "Can't transform laser-data (%s -> %s)",
				                  frame_map_.c_str(),
				                  in[a]->frame.c_str());
				return;
			}
		...
		for (unsigned int i = 0; i < out_data_size; ++i) {
			...
			if (std::isfinite(in[a]->values[i])) {
				...
				// search in 15-neighborhood and itself for occupied pixels in map
